...
(function () {
    ...
    for (i = 0; i < document.getElementsByTagName('svg').length; i += 1) {
        document.getElementsByTagName('svg')[i].style.height = ((minWH - 20) / 3.85) + "px";
        document.getElementsByTagName('svg')[i].style.width = (minWH - 20) + "px";
    }
    ...
    if (Modernizr.deviceorientation) {
        ...
        window.addEventListener('deviceorientation', function (eventData) {
            ...
            if (Math.abs(tiltFB) > 80 && Math.abs(tiltFB) < 100) {
                tiltLR = tiltLR * (Math.abs(90 - Math.abs(tiltFB)) / 10) * (Math.abs(90 - Math.abs(tiltFB)) / 10) * (Math.abs(90 - Math.abs(tiltFB)) / 10);
            }
            ww = window.innerWidth;
            wh = window.innerHeight;
            minWH = Math.min(ww, wh);
